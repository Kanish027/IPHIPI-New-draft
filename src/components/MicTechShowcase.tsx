            ...
            {TECHS.map((tech, i) => (
              <div
                ...
                className={`flex min-h-[70vh] flex-col justify-center border-l-2 pl-6 transition-all duration-500 sm:pl-8 ${
                  i === active
                    ? "border-[#0FF0FC] opacity-100"
                    : "border-zinc-200 opacity-40"
                }`}
              >
                <p className="font-geometric text-sm text-[#0FF0FC]">
                  {String(i + 1).padStart(2, "0")}
                </p>
                <h3 className="mt-3 text-subhead font-semibold tracking-tight">
                  {tech.title}
                </h3>
                <p className="mt-1.5 text-sm font-semibold text-[#0FF0FC]">{tech.spec}</p>
                ...
                <div className="mt-8 max-w-sm">
                  <div className="flex items-center justify-between text-xs">
                    ...
                    <span className="font-geometric font-semibold text-[#0FF0FC]">{tech.meter.value}</span>
                  </div>
                  <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-zinc-100">
                    <div
                      className="h-full rounded-full bg-[#0FF0FC] transition-[width] duration-700"
                  ...
                  {TECHS.map((tech, i) => (
                    <span
                      ...
                      className={`h-1.5 rounded-full transition-all duration-300 ${
                        i === active ? "w-6 bg-[#0FF0FC]" : "w-1.5 bg-white/40"
                      }`}
                    />
                  ))}
                </div>
